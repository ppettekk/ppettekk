<!-- ============================================================= -->
<!-- 1. HEADER                                                      -->
<!-- ============================================================= -->

<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12,15,20&height=220&section=header&text=Petr%20Shuvarikov&fontSize=48&fontColor=ffffff&fontAlignY=32&desc=Full-Stack%20Developer%20%7C%20Telegram%20Bots%20%7C%20Payment%20Integrations&descSize=17&descAlignY=54&animation=fadeIn" alt="header" />

<a href="https://github.com/ppettekk">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=23&pause=1000&color=8B5CF6&center=true&vCenter=true&width=760&lines=Full-Stack+Developer+%7C+Freelancer;Telegram+Bots+%2B+Payment+Integrations;Web+Automation+%26+Scraping;Python+%C2%B7+FastAPI+%C2%B7+React+%C2%B7+Vue;VPS+%C2%B7+nginx+%C2%B7+systemd+%C2%B7+ESP32" alt="typing" />
</a>

<br/>

<img src="https://img.shields.io/badge/RANEPA-International%20Relations%20%26%20Business-6366F1?style=for-the-badge&logo=academia&logoColor=white&labelColor=1a1a2e" alt="university" />
<img src="https://img.shields.io/badge/Self--Employed-Freelance%20Developer-8B5CF6?style=for-the-badge&logo=briefcase&logoColor=white&labelColor=1a1a2e" alt="status" />

<br/><br/>

<a href="https://t.me/ppettekk">
  <img src="https://img.shields.io/badge/Telegram-2CA5E0?style=for-the-badge&logo=telegram&logoColor=white" alt="telegram" />
</a>
<a href="mailto:shuvarikov.petr@gmail.com">
  <img src="https://img.shields.io/badge/Email-7C3AED?style=for-the-badge&logo=gmail&logoColor=white" alt="email" />
</a>
<a href="https://github.com/ppettekk">
  <img src="https://img.shields.io/badge/GitHub-18181B?style=for-the-badge&logo=github&logoColor=white" alt="github" />
</a>

<br/><br/>

<img src="https://komarev.com/ghpvc/?username=ppettekk&style=for-the-badge&color=8B5CF6&label=PROFILE+VIEWS" alt="views" />

</div>

---

<!-- ============================================================= -->
<!-- 2. ABOUT                                                       -->
<!-- ============================================================= -->

## About

Full-stack developer and student at RANEPA. Working as an independent freelancer, I build production systems tailored to client needs: Telegram bots with payment processing and fiscal receipts, web automation and scrapers, and full-stack applications. I value direct communication and shipping working results fast over endless discussion.

- **Telegram ecosystem** — bots with aiogram and python-telegram-bot, WebApps, private channels, invite links.
- **Payment integrations** — YooKassa, Platega, fiscal receipts (54-FZ), recurring payments, promo codes.
- **Web automation** — reverse-engineering mobile APIs, scrapers, userscripts and browser extensions, export to Excel.
- **Full-stack + deployment** — React / Vue 3 / Angular, FastAPI, ModX; deployed on VPS via nginx + systemd.
- **Hardware** — ESP32 and HUB08 LED matrices for real-time displays.

### Open To

`Freelance Projects` · `Bot Development` · `Payment Integrations` · `Web Automation` · `Collaborations`

---

<!-- ============================================================= -->
<!-- 3. TECH STACK                                                  -->
<!-- ============================================================= -->

## Tech Stack

<div align="center">

**Languages**

<img src="https://skillicons.dev/icons?i=python,ts,js,cpp&theme=dark" alt="languages" />

**Frontend**

<img src="https://skillicons.dev/icons?i=react,vue,angular,html,css&theme=dark" alt="frontend" />

**Backend & Databases**

<img src="https://skillicons.dev/icons?i=fastapi,nodejs,postgres,sqlite&theme=dark" alt="backend" />
<br/>
<sub>aiogram · python-telegram-bot · ModX CMS</sub>

**Infra, DevOps & Tooling**

<img src="https://skillicons.dev/icons?i=linux,nginx,docker,git,github,vscode&theme=dark" alt="devops" />
<br/>
<sub>VPS · systemd · YooKassa · Platega</sub>

**Hardware / IoT**

<img src="https://skillicons.dev/icons?i=arduino,cpp&theme=dark" alt="hardware" />
<br/>
<sub>ESP32 · HUB08 LED matrix</sub>

</div>

---

<!-- ============================================================= -->
<!-- 4. SPECIALIZATIONS                                             -->
<!-- ============================================================= -->

## Specializations

| Domain | Level | Details |
| :--- | :---: | :--- |
| **Telegram Bots** | ⭐⭐⭐⭐⭐ | aiogram, python-telegram-bot v22, WebApp, private channels, invite links |
| **Payment Integrations** | ⭐⭐⭐⭐⭐ | YooKassa, Platega, fiscal receipts (54-FZ), recurring payments, promo codes |
| **Web Automation & Scraping** | ⭐⭐⭐⭐☆ | reverse-engineering mobile APIs, userscripts, browser extensions, pipelines → Excel |
| **Full-Stack Web** | ⭐⭐⭐⭐☆ | React, Vue 3, Angular, FastAPI, ModX CMS |
| **Deployment / DevOps** | ⭐⭐⭐⭐☆ | VPS, nginx, systemd, Linux server administration |
| **Hardware / IoT** | ⭐⭐⭐☆☆ | ESP32, HUB08 LED matrices, real-time data displays |

---

<!-- ============================================================= -->
<!-- 5. FEATURED PROJECTS                                           -->
<!-- ============================================================= -->

## Featured Projects

<details>
<summary><b>Grindy — Gig Aggregator for Teenagers</b></summary>

<br/>

Concept and technical implementation of a gig/job aggregator for teenagers with a Telegram WebApp interface.

| | |
| :--- | :--- |
| **Stack** | aiogram · React (WebApp) · PostgreSQL |
| **Format** | Telegram Mini App on top of a bot |
| **Scope** | Full spec, architecture, WebApp frontend |
| **Repository** | [github.com/ppettekk/grindy](https://github.com/ppettekk/grindy) |

A bot + WebApp combo: a feed of gigs with filters inside the Mini App interface, backed by aiogram and Postgres.

</details>

<details>
<summary><b>Pamyat-Naroda Scraper — WWII Archive Parser</b></summary>

<br/>

A scraper for the pamyat-naroda.ru memorial database via reverse-engineering the mobile JSON API, with export to Excel and photos.

| | |
| :--- | :--- |
| **Stack** | Python · requests · openpyxl |
| **Method** | Reverse-engineered mobile JSON API |
| **Output** | Excel tables + downloaded photos/scans |
| **Repository** | [github.com/ppettekk/pamyat-naroda-scraper](https://github.com/ppettekk/pamyat-naroda-scraper) |

Bypassing web limitations through the app's mobile API, with structured export of records and documents to Excel alongside images.

</details>

<details>
<summary><b>Faceit LED Panel — CS2 Stats Display on ESP32</b></summary>

<br/>

A hardware display that shows Faceit/CS2 stats in real time on an LED matrix.

| | |
| :--- | :--- |
| **Stack** | ESP32 · C++ · HUB08 LED matrix · Faceit API |
| **Type** | IoT / hardware |
| **Features** | Pulls Faceit stats via API and renders them on the matrix |
| **Repository** | [github.com/ppettekk/faceit_arduino](https://github.com/ppettekk/faceit_arduino) |

The ESP32 periodically polls the Faceit API and renders ELO/stats onto a HUB08 matrix — a physical desk dashboard.

</details>

---

<!-- ============================================================= -->
<!-- 6. EXPERIENCE                                                  -->
<!-- ============================================================= -->

## Experience

### Freelance Developer (Self-Employed)
`2023 — Present`

Building bots, automations, and web applications on a contract basis. Sourcing clients through Avito, Kwork, and Telegram channels.

- Turnkey Telegram bots with payment processing and fiscal receipts (catalog, payment, access delivery).
- Web automation: scrapers, userscripts, and browser extensions tailored to client tasks.
- Full-stack development in React / Vue / Angular + FastAPI, with ModX integrations.
- Deployment and maintenance on VPS: nginx, systemd, server configuration.
- Scoping requirements, client negotiations, and contracts — from proposal to delivery.

`Python` `aiogram` `FastAPI` `PostgreSQL` `React` `Vue` `nginx` `Platega` `YooKassa`

---

<!-- ============================================================= -->
<!-- 7. ACHIEVEMENTS                                                -->
<!--    ↓ replace with real ones — placeholders below               -->
<!-- ============================================================= -->

## Achievements

<div align="center">

| Recognition | Details |
| :--- | :--- |
| 🚀 **Systems Running in Production** | Bots with payments and fiscal receipts, live for real clients |
| ⭐ **End-to-End Delivery** | From spec and contract to deployment and maintenance |
| 🛠️ **Hardware + Software** | Own IoT projects on ESP32 |

</div>

---

<!-- ============================================================= -->
<!-- 8. CERTIFICATIONS                                              -->
<!-- ============================================================= -->

## Certifications

<div align="center">

**HackerRank**

<a href="https://www.hackerrank.com/certificates/8035cc62d047">
  <img src="https://img.shields.io/badge/HackerRank-Python%20(Basic)-2EC866?style=for-the-badge&logo=hackerrank&logoColor=white" alt="hackerrank-python-basic" />
</a>
<a href="https://www.hackerrank.com/certificates/f74734ef3cc2">
  <img src="https://img.shields.io/badge/HackerRank-Software%20Engineer%20Intern-2EC866?style=for-the-badge&logo=hackerrank&logoColor=white" alt="hackerrank-software-engineer-intern" />
</a>

<br/>

<sub>Verify → <a href="https://www.hackerrank.com/certificates/8035cc62d047">Python (Basic)</a> · <a href="https://www.hackerrank.com/certificates/f74734ef3cc2">Software Engineer Intern</a></sub>

**Harvard University**
<a href="https://cs50.harvard.edu/certificates/53586915-892f-49c9-86eb-d5e17c075479">
  <img src="https://img.shields.io/badge/HarvardX-CS50W%20Web%20Programming-A51C30?style=for-the-badge&logo=harvard&logoColor=white" alt="cs50w" />
</a><sub>CS50’s Web Programming with Python and JavaScript · Verify → cs50.harvard.edu/certificates</sub>

</div>

---

<!-- ============================================================= -->
<!-- 9. GITHUB ANALYTICS                                            -->
<!-- ============================================================= -->

## GitHub Analytics

<div align="center">

<img height="180em" src="https://streak-stats.demolab.com?user=ppettekk&hide_border=true&background=0d1117&stroke=8B5CF6&ring=A855F7&fire=8B5CF6&currStreakLabel=8B5CF6&sideLabels=c9d1d9&currStreakNum=ffffff&sideNums=ffffff&dates=8b949e" alt="streak" />

<br/>


</div>

---

<!-- ============================================================= -->
<!-- 10. GITHUB TROPHIES                                            -->
<!-- ============================================================= -->


<!-- ============================================================= -->
<!-- 11. CONTRIBUTION ACTIVITY                                      -->
<!-- ============================================================= -->

## Contribution Activity

<div align="center">

<img width="100%" src="https://github-readme-activity-graph.vercel.app/graph?username=ppettekk&bg_color=0d1117&color=8B5CF6&line=A855F7&point=ffffff&area=true&area_color=6366F1&hide_border=true" alt="activity-graph" />

</div>

---

<!-- ============================================================= -->
<!-- 12. CONTRIBUTION SNAKE                                         -->
<!-- ============================================================= -->

## Contribution Snake

<div align="center">

<img width="100%" src="https://raw.githubusercontent.com/ppettekk/ppettekk/output/github-contribution-grid-snake-dark.svg" alt="snake" />

</div>

---

<!-- ============================================================= -->
<!-- 13. CURRENT FOCUS                                              -->
<!-- ============================================================= -->

## Current Focus

```yaml
building:
  - Telegram bots with payments and fiscal receipts
  - Web automation: extensions, userscripts, scrapers
  - Marketplace integrations (Yandex Market, digital goods)

learning:
  - Deeper FastAPI and async architecture
  - Frontend: Vue 3 / Angular on real-world tasks

exploring:
  - IoT on ESP32 and real-time dashboards
  - Automation of client business processes

open_to:
  - Freelance projects (bots, payments, automation)
  - Collaborations and interesting technical challenges
```

---

<!-- ============================================================= -->
<!-- 14. CONNECT                                                    -->
<!-- ============================================================= -->

## Connect

<div align="center">

<a href="https://t.me/ppettekk">
  <img src="https://img.shields.io/badge/Telegram-2CA5E0?style=for-the-badge&logo=telegram&logoColor=white" alt="telegram" />
</a>
<a href="mailto:shuvarikov.petr@gmail.com">
  <img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="gmail" />
</a>
<a href="https://github.com/ppettekk">
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="github" />
</a>

</div>

---

<!-- ============================================================= -->
<!-- 15. FOOTER                                                     -->
<!-- ============================================================= -->

<div align="center">

*"Working software in production beats a pretty demo."*

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12,15,20&height=140&section=footer" alt="footer" />

</div>
